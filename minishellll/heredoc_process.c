...
/*   Updated: 2025/08/26 13:51:09 by merilhan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_h_built_expand(t_redirection *c_rdr, t_heredoc_data *data,
		t_env *env_list)
{
	int	pipefd[2];

	if (c_rdr->no_expand)
		data->heredoc_content = read_single_block(c_rdr->filename);
	else
		data->heredoc_content = read_heredoc_expand(c_rdr->filename, env_list);
...
char	*read_heredoc_expand(char *delimiter, t_env *env_list)
{
	t_heredoc_buffer	buf;
	char				*result;
