...
/*   Updated: 2025/08/26 14:13:43 by merilhan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_token							*tokenizer_get_next_token(
...
void							finalize_cmd(t_parser *cmd, int argc,
									t_all *data);
...
void							init_redirection(t_redirection *redir,
									t_redir_type type, char *filename,
									int no_expand);
void							ft_temp_next(t_redirection *redir,
									t_parser *cmd);
...
int								should_split_expansion(char *original_token,
									char *expanded);
void							redir_control_state(t_token *token,
									t_parser *cmd, t_all *all, int result);
void							ft_redir_heredoc(t_token *token, t_parser *cmd);
void							ft_loop(t_token **tokens, t_parser *cmd,
									int *argc, t_all *data);
...
void							ft_not_executable(t_parser *cmd,
									t_exec_data *data, char *exec_path);
void							ft_permission_denied(t_parser *cmd,
									t_exec_data *data);
void							ft_directory(t_parser *cmd, t_exec_data *data);
void							ft_not_directory(t_parser *cmd,
									t_exec_data *data);
...
int								execute_cmds_2(t_parser *cmd_list,
									t_exec_data *data, t_env **env_list,
									int *pid_len);
int								ft_data_pids(t_parser *cmd_list,
									t_exec_data *data, t_env **env_list);
void							finish_fd(t_parser *cmd_list,
									t_exec_data *data);
int								process_command(t_parser *cmd,
									t_exec_data *data, t_env **env_list);
...
char							*check_cmd_in_dir(const char *dir_start,
									int len, char *cmd);
char							*copy_dir(const char *dir_start, int len);
char							*join_path(char *dir, char *file);
int								ft_builtin_call_2(t_parser *cmd,
									t_env **env_list);
int								ft_builtin_call_3(t_parser *cmd,
									t_env **env_list);
...
void							process_quote_char(char *str, int *i,
									char *quote_char);
char							*push_char_res(char *result, char c,
									int *res_len, int *res_cap);
char							*append_string_res(char *result,
									char *to_append, int *res_len,
									int *res_cap);
...
int								h_loop(t_parser *cmd, t_heredoc_data *h_data,
									t_env *env_list);
int								ft_h_built_expand(t_redirection *c_rdr,
...
int								check_delimiter(t_heredoc_buffer *buf,
									const char *delimiter);
int								process_line(t_heredoc_buffer *buf,
...
t_token							*tokenizer_get_next_token_2(
									t_tokenizer *tokenizer);
...
char							*heredoc_sig_handle(
									t_heredoc_buffer *buf, char *res);
